...
pi=3.141592
valvec=np.logspace(-18,-13.5,1000)
...
for val in range(0,len(valvec)):
	...
	amp=sp.AtmosphericNeutrinoOscillationProbability(0,0,E,param.PI,param,pg,ug,eig_dcy,True,False)
...
axes[0].set_ylim([0,1])
axes[0].set_xlabel(r"$\lambda_2$ (eV)")
...
axes[2].plot(fracvec,appampvec,color="red",lw=0)
axes[2].set_xlabel(r"$\lambda_2/\Delta$ (eV/eV)")
axes[1].set_yscale('log')
axes[2].set_xlim([fracvec[0],fracvec[-1]])
axes[2].tick_params(axis='x', pad=10)
for tl in axes[2].get_yticklabels():
    tl.set_color('r')
...
textstr = '$\Delta=%.2e$  (eV)\n$E=%.2e$  (eV)\n'%(delta,E)+r'$\theta=%.2f$  (rad)'%(theta)+'\n$\phi=%.2f$  (rad)\n$\lambda_1=%.2e$  (eV)\n$V=%.2e$'%( phi, eig_dcy[0],potential)

axes[0].text(bb.x0+0.01, bb.y0+0.7, textstr, transform=axes[0].transAxes, fontsize=20,verticalalignment='top', bbox=props)
