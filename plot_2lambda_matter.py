...
data2=np.load('lambda2_matter.npz')
...
#if (numcomp==False):
plotl1= axes[0].plot(xax_vector,diag_vec,color="black",label=r"$\lambda 1$",lw=0,ls='-')
plotl2= axes[0].plot(xax_vector,diag_vec,color="black",label=r"$\lambda 2$",lw=0,ls='--')
...
axes[0].set_xlabel(r"$\lambda$ [TeV]")
...
axes[2].set_xlabel(r"$\lambda/\Delta$ [TeV/TeV]")
...
pretty_exp_note(r'$E$','TeV',E/param.TeV,last=True)]
#pretty_exp_note(r'$\lambda_1$','TeV',l1/param.TeV),
#pretty_exp_note(r'$\lambda_2$','TeV',l2/param.TeV,last=True)]
textstr=''.join(textlist)


axes[0].text(0.47, 0.295, textstr, transform=axes[0].transAxes, fontsize=16,verticalalignment='top', bbox=props)
