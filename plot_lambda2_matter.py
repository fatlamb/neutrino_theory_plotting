...
data=np.load('lambda2_matter.npz')
...
if (numcomp==False):
	...
	axes[0].plot(xax_vector,diag_vec,color="black",label="",lw=0,ls='-.')
axes[0].plot(xax_vector,num_vec,color="purple",label="Numerical Solution",lw=2,ls='-')
...
axes[0].set_xlabel(r"$\lambda_2$ [TeV]")
...
if (numcomp==False):
	...
	axes[1].plot(xax_vector,np.absolute(np.subtract(pert_vec2,num_vec)),color="green",lw=2,ls='-.')
	axes[1].plot(xax_vector,np.absolute(np.subtract(O1_vec,num_vec)),color="green",lw=2,ls='-')
#axes[1].plot(xax_vector,np.absolute(np.subtract(diag_vec,num_vec)),color="pink",lw=2,ls='-')
...
axes[2].plot(frac_vec,np.ones(len(frac_vec)),color="red",lw=0)
axes[2].set_xlabel(r"$\lambda_2/\Delta$ [TeV/TeV]")
axes[2].set_xlim([frac_vec[0],frac_vec[-1]])
axes[2].tick_params(axis='x', pad=10)
axes[1].set_yscale('log')
...
for ax in enumerate(single_axis):
	...
	#legends.append(ax[1].legend(bbox_to_anchor=(0.72, 0.3225),bbox_transform=plt.gcf().transFigure, shadow=False,fancybox=True))
	legends.append(ax[1].legend(loc="lower left", shadow=False,fancybox=True))
...
pretty_exp_note(r'$\lambda_1$','TeV',l1/param.TeV,last=True)]
#pretty_exp_note(r'$\lambda_2$','TeV',l2/param.TeV,last=True)]
textstr=''.join(textlist)


axes[0].text(0.47, 0.36, textstr, transform=axes[0].transAxes, fontsize=16,verticalalignment='top', bbox=props)
